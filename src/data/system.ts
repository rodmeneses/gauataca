...
export const HANDOFF_NOTES: { h: Localized; items: Localized[] }[] = [
  ...
  { h: { es: 'Tablas sugeridas', en: 'Suggested tables' }, items: [
    ...
    { es: 'songs(id, title, genre, key, bpm, duration, youtube_url, spotify_url, apple_music_url, last_rehearsed_at) · song_links(song_id, label, url, position) · takes(event_id, song_id, url, n)', en: 'songs(id, title, genre, key, bpm, duration, youtube_url, spotify_url, apple_music_url, last_rehearsed_at) · song_links(song_id, label, url, position) · takes(event_id, song_id, url, n)' },
